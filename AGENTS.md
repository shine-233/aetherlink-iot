...
- 数据库迁移上界**以代码为准，不要相信本文件里的数字**（本条曾长期写着 82、后写 93，
  实际已推进到 102；每次改动都需重新核对）。修改迁移前必须三处同时核对：
  ...
  2026-09-15 实测快照：`VERSION_NUMBER=102`，`backend/sql/` 最大为 `102.sql`（① ② 一致）。
  **注意：全新空库用 `initialize.CheckVersion` 跑通全链的验证只做到过 93（`sys_version=93`，
  114 张表）；94–102 尚未做过同等的全新库全链验证**，不得据 ①② 一致就宣称 `sys_version` 可达 102。
