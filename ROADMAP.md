...
| P1.3 | Widget 与 SCADA 基础层 | `partial` | `未验证` | ~~编辑器未挂路由~~（2026-09-14 已挂）；~~widget schema 无真实字段~~（2026-09-14 已闭环 `36fd6da`）；~~真实下发联调~~（**2026-09-19 已闭环**：63 号 3/3 全绿，HMAC 确认令牌→真实 MQTT 命令下发→设备回执→审计 success；顺带修 115.sql 确认令牌列过短致确认后控制 100% 失败）；剩余：**仅画布编辑器浏览器 E2E** | `scada_postgres_test.go`、`adeaf80`、`docs/validation/2026-09-19-p13-scada-dispatch-p23-firstscreen-evidence.md` |
...
**统计：`done` 9 项（P0.2, P0.3, P0.4, P0.5, P0.6, P1.1, P1.2, P1.6, P2.2） / `partial` 7 项 / `pending` 1 项（P2.3 压测子项、P3 多数子项）。**
