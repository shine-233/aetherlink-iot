...
| TB-15 | 实体名冲突策略 | 4.3.0 `#14118` | **已实现（2026-09-17，由并行工作流按本条目落地；本轮验收并修两处缺陷）**：`model/conflict_policy.go` 提供 FAIL（默认）/ RENAME / IGNORE / UPDATE / ALLOW 五档与 `NormalizeConflictPolicy`（含 `reject`/`skip`/`overwrite`/`merge`/`auto_rename` 别名）；已接入 **设备 / 设备配置（=产品）/ 看板 / 资产** 四条创建路径及批量创建，API 支持 body 与 query 双通道传 `conflict_policy`。**本轮修复**：① **RENAME 在边界上必然失败**——`device_configs.name` 是 `varchar(99)` 而 HTTP 校验是 `max=99`，原名满长时追加 ` (1)` 必超长被 PG 拒绝，等于 RENAME 在其唯一该起作用的场景退化成 DB 错误；已给更名算法加 `maxLength` 并**按 rune 截断**（按字节截断会把中文砍成半个汉字，产出非法 UTF-8）；影响 5 个调用点。② **策略块插在 JSON 校验之前**，导致既有用例 `TestDeviceConfigCreateRejectsInvalidJSONFieldsBeforeDAL`（契约：非法输入不得触达 DAL）空指针 panic；已把策略块移到 JSON 校验之后，并把 `deviceconfig.Name` 赋值一起下移（否则改名静默不生效）。新增 8 条长度边界用例 | `未验证` | 剩余：**前端未接**（`frontend/src` 对 `conflict_policy` 0 命中，界面无选择项）；**无运行期证据**（未在活栈上用真实 HTTP 验证四种策略）；并发同名存在竞态窗口（查重与插入之间无锁/无唯一约束兜底）；`allow` 放行后的下游取用语义未追查 | S–M | **已落地，仅需补前端入口与联调** |
