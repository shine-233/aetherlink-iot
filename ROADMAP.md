...
| TP-8 | 设备分组统计、模拟遥测数据初始化/发送接口 | 1.2.3 / 1.2.2 | 需逐项核对（`device_group` 有代码，统计与模拟发送未确认） | `未实现`/`未接线` | 无 | S | **建议先核对再立项（高）**：可能是小改动即可补齐 |

### 7.3 立项优先级建议（综合两平台）

**第一梯队（建议立即立项，投入小或刚需）**

1. `TP-4` 设备诊断 / GMQTT 管理界面 / Topic 映射页——后端已有，纯补 UI，直接改善"四面一致"。
2. `TP-8` 设备分组统计 + 模拟遥测数据接口——先核对现状，可能是 S 级改动。
3. `TB-1` 告警规则 2.0——工业刚需，可复用既有告警链路。
