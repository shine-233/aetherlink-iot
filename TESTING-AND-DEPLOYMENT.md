...
## Postgres 18 数据卷迁移（从 ≤17 旧部署升级）

postgres:18+ 官方镜像把数据目录从 `/var/lib/postgresql/data` 移到 `/var/lib/postgresql/18/docker`，compose 挂载点已同步改为 `/var/lib/postgresql`。

- **全新安装**：无感，直接可用。
- **旧部署升级**：命名卷 `postgres-data` 里的存量数据在 `<volume>/data`；更换挂载点后新集群会重新 initdb，等于丢库。二选一：
  1. 固定镜像为 `postgres:17-alpine`（同时把挂载点改回旧路径），延后迁移；
  2. 停栈后用临时 postgres:17 容器把数据 pg_dump，再以新挂载点起 postgres:18 并 pg_restore。
...
| GOTP_OPENAPI_KEY_AUTHORITY | OpenAPI x-api-key 等效权限（2026-08 起默认 TENANT_USER 最小权限）；open_api_keys 表无独立权限字段，所有 Key 共用该值 | 需要 OpenAPI 写能力时显式上调为 TENANT_ADMIN |
