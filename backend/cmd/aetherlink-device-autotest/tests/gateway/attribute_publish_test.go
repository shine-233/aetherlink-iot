...
// 文件用途：验证网关属性嵌套上报在平台中的入库效果。
// 核心逻辑：构造 gateway_data、sub_device_data 和 sub_gateway_data 属性 payload，发布后按目标 device_id 查询属性表。
// 关键注意事项：测试主要断言记录存在，未对所有属性值做强校验，不应等同于完整字段级证明。
// 重构建议：可复用 validator 对每个网关层级做字段值比对，并补齐缺失拓扑时的 skip 记录。

/*
Purpose: 验证网关属性嵌套上报在平台中的入库效果。
Core logic: 构造 gateway_data、sub_device_data 和 sub_gateway_data 属性 payload，发布后按目标 device_id 查询属性表。
Important notes: 测试主要断言记录存在，未对所有属性值做强校验；审查时不要把它等同于完整字段级证明。
Refactor suggestion: 可复用 validator 对每个网关层级做字段值比对，并补齐缺失拓扑时的 skip 记录。
*/
package tests
...
func TestGatewayAttributePublish(t *testing.T) {
	...
	t.Run("Verify_Gateway_Attributes", func(t *testing.T) {
		...
		for key := range gatewayData {
			...
			require.NoError(t, err)
			require.NotNil(t, record, "No attribute data found for gateway key: %s", key)
			require.NoError(t, utils.ValidateAttributeData(gatewayData, record.Key, record.BoolV, record.NumberV, record.StringV),
				"Gateway attribute value mismatch for key: %s", key)
	...
	if len(cfg.Gateway.SubDevices) > 0 {
		t.Run("Verify_SubDevice_Attributes", func(t *testing.T) {
			...
			for key := range subDevAttrs {
				...
				require.NoError(t, err)
				require.NotNil(t, record, "No attribute data found for sub-device key: %s", key)
				require.NoError(t, utils.ValidateAttributeData(subDevAttrs, record.Key, record.BoolV, record.NumberV, record.StringV),
					"Sub-device attribute value mismatch for key: %s", key)
	...
	if len(cfg.Gateway.SubGateways) > 0 {
		t.Run("Verify_SubGateway_Attributes", func(t *testing.T) {
			...
			for key := range subGwAttrs {
				...
				require.NoError(t, err)
				require.NotNil(t, record, "No attribute data found for sub-gateway key: %s", key)
				require.NoError(t, utils.ValidateAttributeData(subGwAttrs, record.Key, record.BoolV, record.NumberV, record.StringV),
					"Sub-gateway attribute value mismatch for key: %s", key)
		...
		if len(cfg.Gateway.SubGateways[0].SubDevices) > 0 {
			t.Run("Verify_SubGateway_SubDevice_Attributes", func(t *testing.T) {
				...
				for key := range subDevAttrs {
					...
					require.NoError(t, err)
					require.NotNil(t, record, "No attribute data found for sub-gateway's sub-device key: %s", key)
					require.NoError(t, utils.ValidateAttributeData(subDevAttrs, record.Key, record.BoolV, record.NumberV, record.StringV),
						"Sub-gateway's sub-device attribute value mismatch for key: %s", key)
