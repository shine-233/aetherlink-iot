...
// 文件用途：验证多层网关在部分数据缺失组合下仍能正确入库。
// 核心逻辑：构造只含顶层网关、只含子网关自有数据、只含子设备数据以及缺 gateway_data 的组合 payload。
// 关键注意事项：这些是边界组合测试，只证明已配置拓扑上的若干正向路径，不代表所有缺字段组合均已覆盖。
// 重构建议：可用表驱动生成组合矩阵，并将期望入库 device_id/key 明确列入测试表。

/*
Purpose: 验证多层网关在部分数据缺失组合下仍能正确入库。
Core logic: 构造只含顶层网关、只含子网关自有数据、只含子设备数据以及缺 gateway_data 的组合 payload，并查询目标记录。
Important notes: 这些是边界组合测试，证明的是已配置拓扑上的若干正向路径，不代表所有嵌套缺字段组合都已覆盖。
Refactor suggestion: 可用表驱动生成组合矩阵，并将期望入库 device_id/key 明确列入测试表。
*/
package tests
...
func TestMultiLayerGateway_OnlyTopGatewayData(t *testing.T) {
	...
	for key := range gatewayData {
		...
		require.NoError(t, err)
		require.NotEmpty(t, records, "No telemetry data found for gateway key: %s", key)
...
func TestMultiLayerGateway_OnlySubGatewayData(t *testing.T) {
	...
	require.NotEmpty(t, cfg.Gateway.SubGateways[0].SubDevices, "Test requires sub-gateway sub-device configuration")
	...
	if len(subGw.SubDevices) > 0 {
		...
		require.NoError(t, err)
		require.NotEmpty(t, records, "No telemetry data found for sub-gateway's sub-device")
...
func TestMultiLayerGateway_OnlySubGatewayOwnData(t *testing.T) {
	...
	require.NoError(t, err)
	require.NotEmpty(t, records, "No telemetry data found for sub-gateway")
...
func TestMultiLayerGateway_OnlyTopSubDeviceData(t *testing.T) {
	...
	require.NoError(t, err)
	require.NotEmpty(t, records, "No telemetry data found for sub-device")
...
func TestMultiLayerGateway_MixedDataCombinations(t *testing.T) {
	...
	// 测试用例1：顶层网关 + 子网关（无 gateway_data）+ 子网关的子设备
	t.Run("TopGateway_And_SubGatewaySubDevice_NoGatewayData", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.NotEmpty(t, records, "No data for top gateway")
...
func TestMultiLayerGateway_AttributeWithoutGatewayData(t *testing.T) {
	...
	if len(subGw.SubDevices) > 0 {
		...
		require.NoError(t, err)
		require.NotNil(t, record, "No attribute data found for sub-gateway's sub-device")
