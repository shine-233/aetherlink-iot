...
// 文件用途：验证平台向网关拓扑各层级下发遥测控制的闭环。
// 核心逻辑：通过 API 向顶层网关、子设备、子网关和子网关子设备下发控制，网关订阅消息并查询控制日志。
// 关键注意事项：用例对接收 payload 做部分结构断言，对日志主要断言状态和 data 包含关系；拓扑缺失会减少实际覆盖。
// 重构建议：可把控制目标抽成表驱动，并为每个层级增加统一的 payload 内容与日志时间窗口断言。

/*
Purpose: 验证平台向网关拓扑各层级下发遥测控制的闭环。
Core logic: 通过 API 向顶层网关、子设备、子网关和子网关子设备下发控制，网关订阅消息并查询遥测控制日志。
Important notes: 用例对接收 payload 做部分结构断言，对日志主要断言状态和 data 包含关系；拓扑缺失会减少实际覆盖。
Refactor suggestion: 可把控制目标抽成表驱动，并为每个层级增加统一的 payload 内容与日志时间窗口断言。
*/
package tests
...
func TestGatewayTelemetryControl(t *testing.T) {
	...
	t.Run("Control_Gateway_Self", func(t *testing.T) {
		...
		require.NotEmpty(t, messages, "Gateway did not receive control message")
		...
		require.NoError(t, err)
		require.NotEmpty(t, logs, "No telemetry set log found for gateway")
	...
	if len(cfg.Gateway.SubDevices) > 0 {
		t.Run("Control_SubDevice", func(t *testing.T) {
			...
			timeout := time.Duration(cfg.Test.WaitMQTTResponseSeconds) * time.Second
			messages := dev.GetReceivedMessages(topics.TelemetryControl(), timeout)
			require.NotEmpty(t, messages, "Gateway did not receive control message for sub-device")
			...
			require.NoError(t, err)
			require.NotEmpty(t, logs, "No telemetry set log found for sub-device")
	...
	if len(cfg.Gateway.SubGateways) > 0 {
		t.Run("Control_SubGateway", func(t *testing.T) {
			...
			timeout := time.Duration(cfg.Test.WaitMQTTResponseSeconds) * time.Second
			messages := dev.GetReceivedMessages(topics.TelemetryControl(), timeout)
			require.NotEmpty(t, messages, "Gateway did not receive control message for sub-gateway")
			...
			require.NoError(t, err)
			require.NotEmpty(t, logs, "No telemetry set log found for sub-gateway")
	...
	if len(cfg.Gateway.SubGateways) > 0 && len(cfg.Gateway.SubGateways[0].SubDevices) > 0 {
		t.Run("Control_SubGateway_SubDevice", func(t *testing.T) {
			...
			timeout := time.Duration(cfg.Test.WaitMQTTResponseSeconds) * time.Second
			messages := dev.GetReceivedMessages(topics.TelemetryControl(), timeout)
			require.NotEmpty(t, messages, "Gateway did not receive control message for sub-gateway's sub-device")
			...
			require.NoError(t, err)
			require.NotEmpty(t, logs, "No telemetry set log found for sub-gateway's sub-device")
