...
## 当前实现状态（以源码为准；静态对账 checked-at：2026-08-04）

本节是源码与旧设计的静态对比，不等于真实通知服务、租户边界或邮件/Webhook 投递
已经在部署环境验收。
...
**当前调用方式：**
```go
ExecuteNotification(groupID, alertJson, tenantID)
```
...
// 3. 执行通知
ExecuteNotification(alarmConfig.NotificationGroupID, alertJson, alarmConfig.TenantID)
```
