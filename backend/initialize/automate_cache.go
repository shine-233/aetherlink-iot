// 文件用途：维护自动化场景触发缓存的读写与回收逻辑，为设备遥测触发链路提供预计算结果。
// 核心逻辑：按单设备和单类设备两种触发维度组织 Redis 键，串联设备缓存、条件组缓存和动作缓存。
// 关键注意事项：缓存键前缀、条件类型和回收顺序共同决定触发结果，修改时必须保持读写双方一致。
// 重构建议：后续可将“键规则”“缓存装配”“回收策略”分层，减少一个结构同时承担过多职责。
...
type AutomateCache struct {
	client    automateCacheStore
	expiredIn time.Duration
	device    AutimateCacheKeyDevice
}

// automateCacheStore is the narrow Redis seam used by automation cache behavior.
// Keeping the interface local makes cache invalidation testable without a live Redis process.
type automateCacheStore interface {
...
// getAutomateCacheKeyBase 生成设备一级缓存键，表示某设备当前挂接的自动化场景集合。
...
// GetDeviceType 返回当前缓存操作绑定的设备维度适配器。
...
// SetDeviceType 切换缓存维度适配器，上层会在单设备和单类设备场景间复用同一实例。
func (c *AutomateCache) SetDeviceType(deviceType AutimateCacheKeyDevice) {
	c.device = deviceType
}

// set 统一将字符串或结构体写入 Redis，并复用调用方指定的过期时间。
...
// scan 统一识别“未命中”“明确无任务”“正常数据”三种缓存返回状态。
...
// deleteCacheBySceneAutId 在当前设备维度下回收动作、条件组和设备一级缓存。
func (c *AutomateCache) deleteCacheBySceneAutId(sceneAutomationId string) error {
	...
	if resultInt == AUTOMATE_CACHE_RESULT_OK {
		for _, groupId := range action.GroupIds {
			...
			for _, v := range groupInfos {
				if v.TriggerSource != nil && v.TriggerConditionType == c.device.GetDeviceTriggerConditionType() {
	...
	if err := c.collectSceneAutomationCacheKeysByScan(sceneAutomationId, &deleteCacheKes); err != nil {
		return err
	}
	// 4 删除缓存
...
// automateDeviceCacheDeleteHandel 清理设备一级缓存中的目标场景引用，并收集需要最终删除的键。
func (c *AutomateCache) automateDeviceCacheDeleteHandel(sceneAutomationId string, ids []string, deleteCacheKes *[]string) error {
	for _, deviceId := range ids {
		...
		automateDeviceInfos, removed := removeSceneAutomationFromDeviceInfos(automateDeviceInfos, sceneAutomationId)
		if !removed {
			continue
		}
...
func (c *AutomateCache) collectSceneAutomationCacheKeysByScan(sceneAutomationId string, deleteCacheKeys *[]string) error {
	if err := c.scanGroupCacheKeysForScene(sceneAutomationId, deleteCacheKeys); err != nil {
		return err
	}
	return c.scanDeviceCacheKeysForScene(sceneAutomationId, deleteCacheKeys)
}

func (c *AutomateCache) scanGroupCacheKeysForScene(sceneAutomationId string, deleteCacheKeys *[]string) error {
	return c.scanKeys(c.getAutomateCacheKeyGroup("*"), func(key string) error {
		var groupInfos DTConditions
		if err := c.client.Get(context.Background(), key).Scan(&groupInfos); err != nil {
			return nil
		}
		for _, condition := range groupInfos {
			if condition.SceneAutomationID == sceneAutomationId {
				*deleteCacheKeys = append(*deleteCacheKeys, key)
				break
			}
		}
		return nil
...
func (c *AutomateCache) scanDeviceCacheKeysForScene(sceneAutomationId string, deleteCacheKeys *[]string) error {
	return c.scanKeys(c.getAutomateCacheKeyBase("*"), func(key string) error {
		var automateDeviceInfos AutomateDeviceInfos
		if err := c.client.Get(context.Background(), key).Scan(&automateDeviceInfos); err != nil {
			return nil
		}
		updatedInfos, removed := removeSceneAutomationFromDeviceInfos(automateDeviceInfos, sceneAutomationId)
		if !removed {
			return nil
		}
...
func removeSceneAutomationFromDeviceInfos(infos AutomateDeviceInfos, sceneAutomationId string) (AutomateDeviceInfos, bool) {
	...
	return updatedInfos, removed
}

func (c *AutomateCache) scanKeys(pattern string, visit func(string) error) error {
...
// SetCacheBySceneAutomationId 按场景维度重建缓存，同时兼顾单设备与单类设备两条索引链。
...
// setCacheBySceneAutId 在当前设备维度下写入条件组缓存、动作缓存和设备一级缓存。
...
// getCacheByDId 在确定缓存键后组装场景、条件组与动作的完整执行视图。
...
// setCache 在当前设备维度下落地条件组、动作和一级设备缓存。
