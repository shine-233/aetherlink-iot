// 文件用途：初始化全局日志系统，并为应用日志与 SQL 日志提供统一输出策略。
// 核心逻辑：依据配置决定控制台、文件或双写输出方式，同时定制彩色控制台格式与文件轮转策略。
// 关键注意事项：日志目录创建、输出目标切换和 SQL writer 复用都会影响启动阶段可观测性，修改时需保持兼容。
// 重构建议：后续可把格式化器、hook 和配置解析拆分，减少单函数承担的初始化分支复杂度。
...
// Format 按彩色控制台格式输出日志，并在末尾附带裁剪后的调用位置信息。
func (*customFormatter) Format(entry *logrus.Entry) ([]byte, error) {
	...
	if entry.HasCaller() {
		filePath := trimProjectCallerPath(entry.Caller.File)
...
// Fire 处理日志条目
// Fire 将格式化后的日志写入目标 writer，供文件 hook 复用。
...
// Format 按纯文本格式输出文件日志，避免 ANSI 颜色码污染日志文件。
...
// trimProjectCallerPath 将绝对路径裁剪为相对项目路径，便于日志阅读。
func trimProjectCallerPath(filePath string) string {
	for _, marker := range []string{"backend"} {
...
// LogInIt 根据配置装配 logrus、文件轮转器和 SQL 独立日志输出。
func LogInIt() error {
...
// 1 - 仅文件输出
// 2 - 文件和控制台都输出
// GetSQLLogWriter 返回供 GORM 使用的 SQL 日志 writer；若未初始化文件输出则退回标准输出。
