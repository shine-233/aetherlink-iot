...
## 当前实现 key 对账（静态，2026-08-04）

下面是当前源码能确认的 producer/consumer 入口；本节不替代后面的历史设计说明。

- 自动化缓存使用 `automate:v3:<scope>:<kind>:<id>`，由
  `initialize/automate_cache.go` 读写，默认 TTL 约 5 分钟，并在场景/条件/动作变化时
  通过对应删除或重建路径失效。
- 数据脚本缓存使用 `{deviceConfigID}_{scriptType}_script`，由
  `backend/internal/processor/cache.go` 与 `initialize/redis_init.go`（GetScriptByDeviceAndScriptType）
  双入口读写；2026-08 起统一使用兜底 TTL（`pkg/constant.CacheFallbackTTL`，30 分钟），
  更新、删除或停用脚本时仍由 `InvalidateCache` 等写路径主动删除（主机制）。
- 设备主缓存（key 为 deviceId）同样自 2026-08 起使用兜底 TTL（30 分钟），
  写路径 `DelDeviceCache`/`DelDeviceCaches` 仍是主要失效手段。
- 设备配置缓存使用 `{deviceConfigID}_config`；通用 JSON helper 在
  `initialize/redis_init.go`，具体 producer/consumer 仍需按调用方核对。
- 旧文档中的 `auto_*`、`auto_devconfig_*` 等 key 作为历史命名记录保留；在没有逐 key
  producer、consumer、TTL、失效路径和恢复证据前，不得按“旧 key”批量清理 Redis。
