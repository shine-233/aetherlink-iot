// 文件用途：连接 MQTT broker 消息与后端上下行处理流程。
// 核心逻辑：管理 MQTT 客户端、订阅 topic、解析设备消息，并发布平台下行响应。
// 关键注意事项：topic 与 payload 是设备协议边界，修改需同步 broker、设备文档和上下行测试。
// 重构建议：可进一步拆分客户端适配、topic 解析和消息分发，便于独立单测错误分支。

// Package mqttadapter bridges MQTT messages into backend uplink/downlink flows.
//
// The adapter parses broker messages, dispatches telemetry/event/status payloads
// to backend processors, and sends downlink responses. Keep topic and payload
// semantics aligned with broker plugin routing.
package mqttadapter
...
// mqttUplinkSourceID returns a fixed-size identity for one protocol-level
// attribute/event report. DeviceID comes from the authenticated cache lookup,
// not from an arbitrary payload field. The durable storage layer later mixes
// this source identity with the resolved target device, so one gateway report
// can safely expand into distinct child envelopes.
func mqttUplinkSourceID(tenantID, deviceID, dataType, messageID string) string {
...
// 调用链：MQTT 回调 -> payload 验证 -> 设备缓存查询 -> topic 类型识别 -> Bus 发布。
// 静态审查重点：这里同时承担验证、缓存查询和消息构造，后续继续膨胀时建议拆成更小的纯函数。
...
// 关键特点：除了把事件投递到 Bus，还会立即发送协议层 ACK，不等待业务处理完成。
func (a *Adapter) HandleEventMessage(payload []byte, topic string) error {
	...
	msg := &UplinkMessage{
		...
		Metadata: map[string]interface{}{
			...
			"source_id":       mqttUplinkSourceID(device.TenantID, device.ID, "event", messageID),
		},
	}
...
// 关键特点：属性上报与事件上报一样，采用“先入 Bus，再立即 ACK”的协议层处理方式。
func (a *Adapter) HandleAttributeMessage(payload []byte, topic string) error {
	...
	msg := &UplinkMessage{
		...
		Metadata: map[string]interface{}{
			...
			"source_protocol": "mqtt",
			"message_id":      messageID,
			"source_id":       mqttUplinkSourceID(device.TenantID, device.ID, "attribute", messageID),
		},
	}
...
// 静态审查重点：topic 解析直接依赖固定层级，协议一旦调整需同步更新这里和测试。
...
// 当前要求 payload 至少包含非空 device_id 和 values；更复杂 schema 仍依赖后续处理链路兜底。
...
// MQTT 协议通过主题前缀判断；如果未来扩展到其他协议，建议把这层判断抽成协议适配策略。
...
// 解析失败时返回空字符串，后续 ACK 仍会继续发送，这一点是协议兼容行为。
...
// 这里只服务于失败路径下的诊断记录，不应把它当成正式业务解析入口。
