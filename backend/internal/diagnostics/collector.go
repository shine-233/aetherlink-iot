// 文件用途：提供后端运行诊断的采集器。
// 核心逻辑：汇总消息处理、错误、队列、Redis、MQTT 和服务健康状态，供排障接口读取。
// 关键注意事项：诊断输出应避免泄露密钥或凭证，并保持并发采集下的数据一致性。
// 重构建议：可将采集器、序列化和保留策略拆分，便于增加指标维度和单元测试覆盖。

// collector.go collects runtime diagnostics for backend processing paths.
//
// Diagnostics support troubleshooting for Redis, telemetry ingestion, MQTT, and
// service health. Keep outputs useful while avoiding secret or credential data.
package diagnostics
