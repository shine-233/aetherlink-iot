// 文件用途：定义 device configs 相关 HTTP 入参、出参和列表查询结构，承接 API 层与模型层的数据契约。
...
type CreateDeviceConfigReq struct {
	...
	DeviceTemplateId *string `json:"device_template_id" validate:"omitempty,max=36"`    // 物模型id
	PayloadSchemaId  *string `json:"payload_schema_id" validate:"omitempty,max=36"`     // 上行数据结构约束id
...
type UpdateDeviceConfigReq struct {
	...
	DeviceTemplateId *string    `json:"device_template_id" validate:"omitempty,max=36"`  // 物模型id
	PayloadSchemaId  *string    `json:"payload_schema_id" validate:"omitempty,max=36"`   // 上行数据结构约束id；空字符串表示解绑
...
type GetDeviceConfigListByPageReq struct {
	PageReq
	DeviceTemplateId *string `json:"device_template_id" form:"device_template_id" validate:"omitempty,max=36"` // 物模型id
