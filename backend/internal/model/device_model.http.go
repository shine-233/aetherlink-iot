// 文件用途：定义 device model 相关 HTTP 入参、出参和列表查询结构，承接 API 层与模型层的数据契约。
...
type CreateDeviceModelReq struct {
	...
	ReadWriteFlag    *string `json:"read_write_flag" validate:"omitempty,max=10,oneof=R W RW"` // 读写标志R-读 W-写 RW-读写
...
type GetDeviceModelListByPageReq struct {
	PageReq
	DeviceTemplateId string  `json:"device_template_id" form:"device_template_id"  validate:"required,max=36"` // 设备物模型ID
