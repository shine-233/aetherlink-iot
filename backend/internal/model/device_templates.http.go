// 文件用途：定义物模型相关 HTTP 入参、出参和列表查询结构，承接 API 层与模型层的数据契约。
...
// 发布单位是 device_config，包含凭证协议配置和它引用的 device_template（物模型+可视化配置）
...
// PublishTemplateReq 发布物模型到市场的业务契约对象（发送给 market-service）
type PublishTemplateReq struct {
	...
	// TemplateDefinition 物模型定义（可视化配置+物模型）
...
// InstallFromMarketReq 从市场安装物模型请求
...
// MarketTemplateListReq 市场物模型列表请求（代理转发用）
...
// MarketTemplateFullData 市场物模型完整数据（从市场下载的完整定义）
...
// TemplateDefinitionPayload 物模型展示定义（可视化配置）
