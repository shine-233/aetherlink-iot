// 文件用途：承载设备编解码脚本处理模块的 cache 逻辑。
// 核心逻辑：围绕脚本缓存、Lua 沙箱执行、输入输出模型和处理器接口实现上下行数据转换，主要围绕 type ScriptCache、func NewScriptCache、func (c *ScriptCache) GetScript、func (c *ScriptCache) InvalidateCache 等声明展开。
