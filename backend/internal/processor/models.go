// 文件用途：承载设备编解码脚本处理模块的 models 逻辑。
// 核心逻辑：围绕脚本缓存、Lua 沙箱执行、输入输出模型和处理器接口实现上下行数据转换，主要围绕 type DecodeInput、type DecodeOutput、type EncodeInput、type EncodeOutput 等声明展开。
