// 文件用途：维护租户用户信息、租户资料和密码更新服务。
// 核心逻辑：读取租户及用户资料，校验邮箱/手机号/密码，并更新租户基础信息。
// 关键注意事项：租户资料更新影响整租户展示和账号安全，权限、密码校验和联系方式唯一性需严格。
// 重构建议：拆分资料更新事务、密码策略和联系方式校验，补齐跨租户、回滚和审计测试。
// users.go exposes tenant account self-service operations.
//
// Purpose: read tenant profile data, update tenant contact/company details, and change tenant account passwords from authenticated user context.
// Core logic: loads the current tenant/user rows, validates ownership and old password checks, hashes new passwords, and preserves existing metadata while applying profile updates.
// Important notes: these paths modify identity data, so nil claims, cross-user email lookups, and password mismatch cases must fail before writes.
// Refactor suggestion: consolidate tenant profile reads and password update validation with sys-user management helpers.
package service
...
func (*UsersService) UpdateTenantInfo(ctx context.Context, userInfo *utils.UserClaims, param *model.UsersUpdateReq) error {
	...
	if param.Email != nil {
		...
		if !strings.EqualFold(email, info.Email) {
			...
			} else if existing != nil && existing.ID != info.ID {
