...
// AttributeCurrentUpsertClause keeps the current attribute table monotonic by
// accepting an incoming row only when it is at least as new as the persisted
// row for the same device/key pair.
func AttributeCurrentUpsertClause() clause.OnConflict {
	return clause.OnConflict{
		...
		Where: clause.Where{Exprs: []clause.Expression{
			clause.Expr{SQL: "EXCLUDED.ts >= attribute_datas.ts"},
		}},
	}
}
