// 文件用途：提供日期起点、格式化、星期映射和场景联动下一次执行时间计算。
// 核心逻辑：按小时、天、周、月和 Cron 表达式解析条件，并返回当前时间之后的下一次执行点。
// 关键注意事项：条件字符串格式是调用契约，切片和时间解析前需要由调用方保证输入符合预期。
// 重构建议：后续可把不同 taskType 的解析器拆成独立函数，并用结构化条件替代脆弱字符串。
package common
