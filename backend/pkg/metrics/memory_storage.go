// 文件用途：提供系统指标历史数据的内存存储实现。
// 核心逻辑：保存当前 CPU/内存/磁盘数据，每隔默认采集间隔记录历史点，并定期清理超出保留期的数据。
// 关键注意事项：该存储只适合短期展示，进程重启后数据会丢失；后台清理 goroutine 会随实例启动。
// 重构建议：后续可支持可配置采样间隔和持久化存储实现，便于生产环境历史趋势分析。
package metrics
