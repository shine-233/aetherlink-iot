// 文件用途：定义 Prometheus 指标集合、历史存储接口和系统资源周期采集。
// 核心逻辑：注册内存、CPU、磁盘、API、错误和响应大小指标，并按定时器刷新当前进程数据。
// 关键注意事项：默认 Prometheus registry 中的指标名是运维契约，重复注册同名指标会导致初始化失败。
// 重构建议：后续可支持自定义 registry 和可停止的采集上下文，降低测试和多实例初始化风险。
