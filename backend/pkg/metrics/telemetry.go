// 文件用途：实现可选产品遥测的状态管理、事件去重和 payload 构造。
// 核心逻辑：根据配置判断是否启用，读取本地状态文件，按注册、升级和心跳桶发送 PostHog 事件。
// 关键注意事项：禁用遥测时必须保持无网络请求；状态文件损坏会阻断本次遥测周期并返回错误。
// 重构建议：后续可抽象事件发送器和状态仓库，并增加重试/退避策略。
package metrics
