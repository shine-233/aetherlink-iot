...
func (*Device) InitDevice(Router *gin.RouterGroup) {
	...
	deviceapi := Router.Group("device")
	{
		...
		// 设备 twin / desired-vs-reported 聚合
		deviceapi.GET("twin/:id", api.Controllers.DeviceTwinApi.HandleDeviceTwin)
		deviceapi.PUT("twin/:id/desired", api.Controllers.DeviceTwinApi.UpsertDeviceTwinDesired)
		// fleet 级 twin drift 可查询索引（枚举一批设备聚合 convergence 分类）
		deviceapi.GET("twin-drift", api.Controllers.DeviceTwinApi.HandleDeviceTwinDriftIndex)

		// 设备调试日志（gmqtt 采集，平台提供读取/开关）
		deviceapi.POST(":id/debug", api.Controllers.DeviceDebugApi.SetDeviceDebug)
		deviceapi.GET(":id/debug/status", api.Controllers.DeviceDebugApi.GetDeviceDebugStatus)
		deviceapi.GET(":id/debug/logs", api.Controllers.DeviceDebugApi.GetDeviceDebugLogs)
		deviceapi.POST(":id/mqtt-debug/session", api.Controllers.DeviceDebugApi.OpenMQTTDebugSession)
		deviceapi.GET(":id/mqtt-debug/session/:session_id", api.Controllers.DeviceDebugApi.GetMQTTDebugSession)
		deviceapi.POST(":id/mqtt-debug/session/:session_id/command", api.Controllers.DeviceDebugApi.ApplyMQTTDebugCommand)
		deviceapi.DELETE(":id/mqtt-debug/session/:session_id", api.Controllers.DeviceDebugApi.CloseMQTTDebugSession)
		deviceapi.GET(":id/connection/diagnostics", api.Controllers.DeviceApi.GetDeviceConnectionDiagnostics)
		deviceapi.GET(":id/onboarding/connection-guide", api.Controllers.DeviceApi.GetDeviceConnectionGuide)
		...
		topicMappingApi := deviceapi.Group("topic-mappings")
		{
			...
			topicMappingApi.POST("dry-run", api.Controllers.DeviceTopicMappingApi.DryRunDeviceTopicMapping)
