...
-- Casbin's policy table is a prerequisite for this migration and must also
-- exist when the numbered chain is applied to a brand-new database.
CREATE SEQUENCE IF NOT EXISTS public.casbin_rule_id_seq;

CREATE TABLE IF NOT EXISTS public.casbin_rule (
	id bigint NOT NULL DEFAULT nextval('public.casbin_rule_id_seq'::regclass),
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_casbin_rule
	ON public.casbin_rule (ptype, v0, v1, v2, v3, v4, v5);
