...
- `backend/pkg/global/global.go` currently declares `VERSION_NUMBER = 60`.
- The numbered migration chain is `1.sql` through `60.sql`, with no missing number. `backend/initialize/pg_init.go` (`CheckVersion`) reads `sys_version` and executes `sql/<n>.sql` in ascending order for every version greater than the database version and up to `60`.
