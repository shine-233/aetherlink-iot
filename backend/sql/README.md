...
- `backend/pkg/global/global.go` currently declares `VERSION_NUMBER = 50`.
- The numbered migration chain is `1.sql` through `52.sql`, with no missing number. `backend/initialize/pg_init.go` (`CheckVersion`) reads `sys_version` and executes `sql/<n>.sql` in ascending order for every version greater than the database version and up to `52`.
...
- A schema snapshot elsewhere in the repository is reference material only; verify the target database version and the numbered migration chain before generating model/query code.
 `51.sql` migrates credential uniqueness to a unique voucher_hash index, and `52.sql` adds the forward_rules table for the data-forward rule engine (third-party HTTP/MQTT delivery of device uplinks).