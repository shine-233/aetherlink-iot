# AetherLink IoT 系统架构图

以下是基于代码分析的 AetherLink IoT 系统架构图：
...
graph TB
    ...
    subgraph DataStorage[数据存储层]
        PostgreSQL[(PostgreSQL 16数据库)]
        Redis[(Redis缓存)]
        TimescaleDB[TimescaleDB扩展（可选）]
        PostgreSQL -. 已安装扩展时启用 .-> TimescaleDB
    end
...
1. **PostgreSQL**：默认主数据库，使用普通 PostgreSQL 表存储系统配置、用户信息和设备遥测数据
2. **Redis**：缓存服务，用于设备状态管理和数据缓存
3. **TimescaleDB（可选）**：PostgreSQL 扩展；安装后可将遥测表转换为 hypertable，未安装时不影响核心能力
...
3. **高性能**：默认使用Redis缓存和PostgreSQL；安装可选TimescaleDB扩展后，可进一步优化时序查询
