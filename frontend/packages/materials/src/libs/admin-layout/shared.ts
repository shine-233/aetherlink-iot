/**
 * 文件用途：提供 AdminLayout 的共享常量和 CSS 变量生成逻辑。
 * 核心逻辑：维护滚动容器 ID、最大 z-index，并将布局 props 转换为 CSS 变量对象。
 * 关键注意事项：CSS 变量名需要与 index.module.css 和组件模板保持一致。
 * 重构建议：可为 CSS 变量生成函数增加输入默认值和边界值测试。
 */
