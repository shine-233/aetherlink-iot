/**
 * 文件用途：提供交互式 Git 提交和提交信息校验命令。
 * 核心逻辑：通过 prompts 收集 Conventional Commits 字段，并调用 git commit 或校验提交消息。
 * 关键注意事项：命令会读取 Git 提交信息并可能执行真实提交，使用前需确认工作区状态。
 * 重构建议：可把提交消息拼装和校验拆成纯函数，便于测试和复用。
 */
