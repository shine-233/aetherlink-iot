/**
 * 文件用途：验证 grid 工具函数的布局处理、校验和边界输入行为。
 * 核心逻辑：构造不同尺寸、坐标和配置组合，断言工具函数输出稳定。
 * 关键注意事项：测试应覆盖异常输入与边界值，避免布局算法只在理想数据下通过。
 * 重构建议：可按 common、responsive、validation 和 layout-algorithm 拆分测试套件。
 */
/**
 * Grid工具函数单元测试
 */
...
describe('Grid Utils Enhanced', () => {
  ...
  describe('checkCollisions', () => {
    it('should detect collisions', () => {
      ...
      expect(result.suggestedPosition).toEqual(
  ...
  describe('findOptimalPosition', () => {
    it('should find optimal position for new item', () => {
      ...
      expect(position).toMatchObject({ col: 4, row: 1 })
    })
  ...
  describe('Performance Monitor', () => {
    it('should track performance metrics', () => {
      ...
      expect(metrics.renderTime).toBe(duration)
