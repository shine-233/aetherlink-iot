...
export interface WidgetConfigSchema {
  type: 'object'
  properties: Record<string, { type: string; title?: string; default?: unknown; enum?: unknown[] }>
  required?: string[]
}
...
export interface WidgetInstance {
  update(data: TelemetryPoint[]): void
  resize(): void
  destroy(): void
}
export interface WidgetRegistry {
  register(definition: WidgetDefinition): void
  resolve(type: string, version?: string): WidgetDefinition | undefined
  list(category?: WidgetDefinition['category']): WidgetDefinition[]
}
