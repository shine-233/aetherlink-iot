# AetherLink IoT 国际化开发指南

## 🌐 项目国际化介绍

AetherLink IoT 前端项目采用了现代化的模块化国际化架构，当前提供 zh-CN、en-US、fr-FR、es-ES 四套语言包，并通过英文兜底保证非核心语言缺键时仍可显示。

### 🏗️ 架构特点

- **模块化管理**: 按功能拆分为21个 JSON 模块，便于维护
- **多语言支持**: 中文（zh-CN）、英文（en-US）、法文（fr-FR）和西班牙文（es-ES）
- **自动合并**: 运行时自动合并所有模块文件
...
│   │   ├── card.json            # 卡片/业务组件文本 (约252-254个键)
│   │   └── ... (其他16个模块)
│   ├── en-us/                   # 英文模块文件 (21个)
│       ├── basic.json
│       ├── common.json
│       ├── page.json
│       └── ... (对应中文模块)
│   ├── fr-fr/                   # 法文模块文件 (21个，英文兜底)
│   └── es-es/                   # 西班牙文模块文件 (21个，英文兜底)
├── locale.ts                    # 语言包合并导出
├── index.ts                     # 国际化入口
└── __tests__/                  # 国际化相关测试
```

仓库根目录 `frontend/.i18nrc.json` 是 i18n Ally 的实际配置；本目录没有单独的
`i18n-ally-config.json` 或 `配置说明.md` 副本。本指南文件本身是
`src/locales/AetherLink国际化开发指南.md`。
...
| `card.json` | 约252-254 | 卡片/业务组件 | 设备、插件、遥测等组件文本 |
| `common.json` | 279 | 通用文本 | 按钮、提示等 |
| `custom.json` | 177 | 自定义功能 | 业务逻辑相关 |
| `route.json` | 121 | 路由相关 | 导航和菜单 |
| 其他模块 | 以当前文件为准 | 专项功能 | 设备、告警、市场、RDI 等 |
| **总计** | **随语言包变动** | **21个模块/语言** | **以 `locale.ts` 自动合并结果为准** |
...
**配置检查**（在 `frontend/` 目录执行）：

1. 确认现有的 `.i18nrc.json` 存在，不要从不存在的 `src/locales/i18n-ally-config.json` 复制。
2. 检查 `pathMatcher` 是否指向 `src/locales/langs/{locale}/*.json`。
3. 修改配置时直接编辑根级 `.i18nrc.json`，不要新建第二份配置文件。
...
**文档维护**: AetherLink IoT 开发团队<br>
**最后更新**: 2025年1月1日<br>
**适用版本**: AetherLink IoT Frontend Community v1.0+
