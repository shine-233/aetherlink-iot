...
describe('useRdiHistory - 纯函数与格式化', () => {
  ...
  describe('历史曲线选择', () => {
    ...
    it('空选择时回退到默认全部序列', async () => {
      ...
      const requestedKeys = (rdiDeviceHistory as unknown as Mock).mock.calls.map(([, params]) => params.key)
    ...
    it('非空选择时只请求并展示用户选择的序列', async () => {
      ...
      expect(
        (composable.historyChartOptions.value as unknown as ChartOptionsStub).series.map((series) => series.name)
      ).toEqual(['T1', 'SW2'])
    })
    ...
    it('为每个客户可见输入保持稳定且互不重复的曲线颜色', () => {
      ...
      const options = composable.historyChartOptions.value as unknown as ChartOptionsStub
      const seriesColors = Object.fromEntries(options.series.map((series) => [series.name, series.lineStyle.color]))
