// 文件用途：实现 `gmqctl gen plugin` 命令的参数解析和插件模板落盘流程。
// 核心逻辑：校验插件名、hook、配置开关和输出目录后，按模板生成主文件、hook 文件和可选配置文件。
// 关键注意事项：生成器会拒绝覆盖已存在文件，调用方需要提前选择空目录或清理旧输出。
// 重构建议：可将全局 flag 状态收敛到参数结构体，降低测试间共享变量带来的串扰风险。
package gen_plugin
