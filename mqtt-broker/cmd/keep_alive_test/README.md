...
这个测试程序用于复现和观察 GMQTT 的 TCP Keep-Alive 问题（每 15 秒发送一次 TCP Keep-Alive ACK 包）。它只建立原始 TCP 连接并输出诊断信息，是供本地开发和故障排查使用的辅助工具，不是 MQTT 客户端，也不能作为生产 MQTT broker 运行或部署。

以下命令均从仓库根目录执行，并使用仓库相对路径；PowerShell、CMD、Git Bash 和常见 Linux/macOS shell 均可按相同目录结构运行。
...
```bash
cd mqtt-broker/cmd/gmqttd
go run . start -c aetherlink.yml
```
...
```bash
# Windows PowerShell
tshark -i 8 -f "tcp port 1883" -w mqtt_keepalive.pcap

# Linux
tshark -i lo -f "tcp port 1883" -w mqtt_keepalive.pcap
```
...
```bash
sudo tcpdump -i lo port 1883 -w mqtt_keepalive.pcap
```

### 3. 运行测试程序

```bash
cd mqtt-broker/cmd/keep_alive_test
go run simple_tcp.go
```
...
如需改成其他 broker 地址，编辑 `simple_tcp.go` 中的 `addr := "127.0.0.1:1883"`。

### 修改状态打印间隔

如需调整观察输出频率，编辑 `simple_tcp.go` 中的 `ticker := time.NewTicker(10 * time.Second)`。

## 问题说明

TCP Keep-Alive 15 秒是 **Go 语言 net 包的默认行为**，不是 GMQTT 的配置问题。

当前仓库以本 README 的抓包说明和 `mqtt-broker/server/server.go` 中的禁用逻辑为准；旧的外部分析文档路径已不再保留在源码树中。
