// 文件用途：维护 config\mqtt.go 所属 broker 包的手写 Go 代码。
...
const (
	...
	SharedSubBalanceRandom    = "random"
	SharedSubBalanceTopicHash = "topic_hash"
)
...
var (
	...
	DefaultMQTTConfig = MQTT{
		...
		SharedSubBalanceStrategy:   SharedSubBalanceRandom,
		AllowZeroLenClientID:       false,
...
type MQTT struct {
	...
	SharedSubBalanceStrategy string `yaml:"shared_subscription_balance_strategy"`
	...
	// AllowAnonymous indicates whether clients may connect without an authentication hook.
...
func (c MQTT) Validate() error {
	...
	if c.SharedSubBalanceStrategy != SharedSubBalanceRandom && c.SharedSubBalanceStrategy != SharedSubBalanceTopicHash {
		return fmt.Errorf("invalid shared_subscription_balance_strategy: %s", c.SharedSubBalanceStrategy)
	}
