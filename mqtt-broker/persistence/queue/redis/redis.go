// 文件用途：维护 persistence\queue\redis\redis.go 所属 broker 包的手写 Go 代码。
...
func (q *Queue) removeQueuedElem(conn redigo.Conn, b []byte) error {
...
func (q *Queue) Read(pids []packets.PacketID) (elems []*queue.Elem, err error) {
	...
	if !q.inflightDrained {
		// 必须先调用 ReadInflight 排空 inflight 消息后才能调用 Read
		return nil, errors.New("must call ReadInflight to drain all inflight messages before Read")
	}
