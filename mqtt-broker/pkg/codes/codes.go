// Package codes 定义 MQTT v3/v5 连接和错误处理相关原因码。
//
// 文件用途：集中维护 MQTT 协议原因码、共享错误实例和错误包装结构。
// 核心逻辑：通过 Code 常量表达协议字节值，并用 Error 携带 reason string 与 user properties。
// 关键注意事项：常量值是外部协议契约，不能为了命名或排序随意修改。
// 重构建议：新增 reason code 时使用表驱动测试锁定数值、错误字符串和共享错误行为。
package codes
