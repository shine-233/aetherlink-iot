// 文件用途: 实现 MQTT CONNECT 控制包结构和编码行为。
// 核心逻辑: 解析协议名、协议版本、凭据、遗嘱数据和 CONNECT 属性。
// 关键注意事项: 认证相关字段和版本处理需要与服务端连接测试保持一致。
// 重构建议: 将 MQTT 3.x 与 MQTT 5 的属性解析拆成更小的 helper。
// connect.go 实现 MQTT CONNECT 控制包结构和编码行为。
//
// 认证字段、协议版本处理和属性解析需要持续对齐服务端/客户端连接测试。
package packets
...
func (c *Connect) String() string {
	return fmt.Sprintf("Connect, Version: %v,"+"ProtocolLevel: %v, UsernameFlag: %v, PasswordFlag: %v, ProtocolName: %s, CleanStart: %v, KeepAlive: %v, ClientID: %s, Username: %s, PasswordLength: %d"+
		", WillFlag: %v, WillRetain: %v, WillQos: %v, WillMsgLength: %d, Properties: %s, WillProperties: %s",
		c.Version, c.ProtocolLevel, c.UsernameFlag, c.PasswordFlag, c.ProtocolName, c.CleanStart, c.KeepAlive, c.ClientID, c.Username, len(c.Password), c.WillFlag, c.WillRetain, c.WillQos, len(c.WillMsg), c.Properties, c.WillProperties)
}
