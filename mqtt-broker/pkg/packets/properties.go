// 文件用途: 实现 MQTT v5 属性的编码与解码。
// 核心逻辑: 为不同控制包读写可变长度属性集合。
// 关键注意事项: 属性解析属于线协议兼容行为，需要互操作性审查。
// 重构建议: 将属性元数据表与二进制读写流程拆分。
// properties.go 实现 MQTT v5 属性的编码与解码。
//
// 属性解析属于线协议兼容行为，变更应由聚焦的控制包测试和互操作性审查支撑。
package packets
...
func (p *Properties) UnpackWillProperties(bufr *bytes.Buffer) error {
	propBufr, err := readPropertyBlock(bufr)
	if err != nil {
		return err
	}
	if propBufr == nil {
	...
	return readProperties(propBufr, p.unpackWillProperty)
}
...
func (p *Properties) Unpack(bufr *bytes.Buffer, packetType byte) error {
	...
	err = readProperties(propBufr, func(propType byte, r *bytes.Buffer) error {
		if !ValidateID(packetType, propType) {
			return codes.ErrProtocol
		}
		return p.unpackProperty(propType, r)
	})
	if err != nil {
		return err
	}
	return p.validateUnpackedProperties()
}
...
func readPropertyBlock(bufr *bytes.Buffer) (*bytes.Buffer, error) {
	...
	return bytes.NewBuffer(bufr.Next(length)), nil
}

func readProperties(bufr *bytes.Buffer, readProperty func(propType byte, r *bytes.Buffer) error) error {
	for {
		propType, err := bufr.ReadByte()
		if err != nil {
			if err == io.EOF {
				return nil
			}
		...
		if err := readProperty(propType, bufr); err != nil {
			return err
		}
	}
}

func (p *Properties) validateUnpackedProperties() error {
...
func (p *Properties) unpackWillProperty(propType byte, r *bytes.Buffer) error {
	var err error
	switch propType {
	case PropWillDelayInterval:
		p.WillDelayInterval, err = propertyReadUint32(p.WillDelayInterval, r, propType, nil)
	case PropPayloadFormat:
		p.PayloadFormat, err = propertyReadBool(p.PayloadFormat, r, propType)
	case PropMessageExpiry:
		p.MessageExpiry, err = propertyReadUint32(p.MessageExpiry, r, propType, nil)
	case PropContentType:
		p.ContentType, err = propertyReadUTF8String(p.ContentType, r, propType, nil)
	case PropResponseTopic:
		p.ResponseTopic, err = propertyReadUTF8String(p.ResponseTopic, r, propType, validResponseTopic)
	case PropCorrelationData:
		p.CorrelationData, err = propertyReadBinary(p.CorrelationData, r, propType, nil)
	case PropUser:
		return p.readUserProperty(r)
	default:
		return codes.ErrMalformed
	}
	return err
...
func (p *Properties) unpackProperty(propType byte, r *bytes.Buffer) error {
	var err error
	switch propType {
	case PropPayloadFormat:
		p.PayloadFormat, err = propertyReadBool(p.PayloadFormat, r, propType)
	...
	case PropSubscriptionIdentifier:
		err = p.readSubscriptionIdentifier(r)
	case PropSessionExpiryInterval:
		p.SessionExpiryInterval, err = propertyReadUint32(p.SessionExpiryInterval, r, propType, nil)
	case PropAssignedClientID:
		p.AssignedClientID, err = propertyReadUTF8String(p.AssignedClientID, r, propType, nil)
	case PropServerKeepAlive:
		p.ServerKeepAlive, err = propertyReadUint16(p.ServerKeepAlive, r, propType, nil)
	case PropAuthMethod:
		p.AuthMethod, err = propertyReadUTF8String(p.AuthMethod, r, propType, nil)
	case PropAuthData:
		p.AuthData, err = propertyReadUTF8String(p.AuthData, r, propType, nil)
	case PropRequestProblemInfo:
		p.RequestProblemInfo, err = propertyReadBool(p.RequestProblemInfo, r, propType)
	case PropWillDelayInterval:
		p.WillDelayInterval, err = propertyReadUint32(p.WillDelayInterval, r, propType, nil)
	case PropRequestResponseInfo:
		p.RequestResponseInfo, err = propertyReadBool(p.RequestResponseInfo, r, propType)
	case PropResponseInfo:
		p.ResponseInfo, err = propertyReadUTF8String(p.ResponseInfo, r, propType, nil)
	case PropServerReference:
		p.ServerReference, err = propertyReadUTF8String(p.ServerReference, r, propType, nil)
	case PropReasonString:
		p.ReasonString, err = propertyReadUTF8String(p.ReasonString, r, propType, nil)
	case PropReceiveMaximum:
		p.ReceiveMaximum, err = propertyReadUint16(p.ReceiveMaximum, r, propType, nonZeroUint16)
	case PropTopicAliasMaximum:
		p.TopicAliasMaximum, err = propertyReadUint16(p.TopicAliasMaximum, r, propType, nil)
	case PropTopicAlias:
		p.TopicAlias, err = propertyReadUint16(p.TopicAlias, r, propType, nonZeroUint16)
	case PropMaximumQoS:
		p.MaximumQoS, err = propertyReadBool(p.MaximumQoS, r, propType)
	case PropRetainAvailable:
		p.RetainAvailable, err = propertyReadBool(p.RetainAvailable, r, propType)
	case PropUser:
		err = p.readUserProperty(r)
	case PropMaximumPacketSize:
		p.MaximumPacketSize, err = propertyReadUint32(p.MaximumPacketSize, r, propType, nonZeroUint32)
	case PropWildcardSubAvailable:
		p.WildcardSubAvailable, err = propertyReadBool(p.WildcardSubAvailable, r, propType)
	case PropSubIDAvailable:
		p.SubIDAvailable, err = propertyReadBool(p.SubIDAvailable, r, propType)
	case PropSharedSubAvailable:
		p.SharedSubAvailable, err = propertyReadBool(p.SharedSubAvailable, r, propType)
	default:
		return codes.ErrMalformed
	}
	return err
...
func (p *Properties) readSubscriptionIdentifier(r *bytes.Buffer) error {
	if len(p.SubscriptionIdentifier) != 0 {
		return codes.ErrProtocol
	}
...
func (p *Properties) readUserProperty(r *bytes.Buffer) error {
	k, err := readUTF8String(true, r)
	if err != nil {
		return codes.ErrMalformed
	}
	v, err := readUTF8String(true, r)
	if err != nil {
		return codes.ErrMalformed
	}
...
func validResponseTopic(u []byte) bool {
	return ValidTopicName(true, u) // [MQTT-3.3.2-14]
}

func nonZeroUint16(u uint16) bool {
	return u != 0
}

func nonZeroUint32(u uint32) bool {
	return u != 0
}
