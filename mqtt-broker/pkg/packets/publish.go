// 文件用途: 实现 MQTT PUBLISH 控制包的编码与解码。
// 核心逻辑: 处理主题名、包标识符、QoS 标志、负载和属性。
// 关键注意事项: PUBLISH 是主要数据通路，变更需要 broker 与控制包测试共同覆盖。
// 重构建议: 拆分标志校验、主题校验以及负载/属性解析逻辑。
package packets
...
func (p *Publish) String() string {
	return fmt.Sprintf("Publish, Version: %v, Pid: %v, Dup: %v, Qos: %v, Retain: %v, TopicName: %s, PayloadLength: %d, Properties: %s",
		p.Version, p.PacketID, p.Dup, p.Qos, p.Retain, p.TopicName, len(p.Payload), p.Properties)
}
