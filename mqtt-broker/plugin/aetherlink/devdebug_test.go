// 文件用途：维护 plugin\aetherlink\devdebug_test.go 所属 broker 包的手写 Go 代码。
...
func TestWriteDeviceDebugLog_RedactsSensitivePayloadFields(t *testing.T) {
	s, err := miniredis.Run()
	if err != nil {
		t.Fatalf("miniredis: %v", err)
	}
	...
	devDebugNow = func() time.Time { return time.Unix(4000, 0) }
	t.Cleanup(func() { devDebugNow = time.Now })
	...
	_, err = WriteDeviceDebugLog(deviceID, DeviceDebugLogEntry{
		Action:    "publish",
		Direction: "up",
		Payload:   `{"temperature":21,"password":"secret","nested":{"access_token":"abc"}}`,
		Outcome:   "ok",
	})
	if err != nil {
		t.Fatalf("write log: %v", err)
	...
	if strings.Contains(entry.Payload, "secret") || strings.Contains(entry.Payload, "abc") {
		t.Fatalf("payload leaked sensitive values: %s", entry.Payload)
	}
	if strings.Count(entry.Payload, deviceDebugRedacted) != 2 {
