// 文件用途：维护 server\client_test.go 所属 broker 包的手写 Go 代码。
...
func TestClient_pollNewMessages_MessageExpiryRemaining(t *testing.T) {
	a := assert.New(t)
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	srv := defaultServer()
	...
	originalExpiry := uint32(10)
	msg := &gmqtt.Message{
		QoS:           packets.Qos1,
		PacketID:      1,
		Topic:         "topic/a",
		Payload:       []byte("payload"),
		MessageExpiry: originalExpiry,
	}
	qs.EXPECT().Read([]packets.PacketID{1}).Return([]*queue.Elem{
		{
	...
	unused, err := c.pollNewMessages([]packets.PacketID{1})
	...
	case p := <-c.out:
		...
		if a.NotNil(pub.Properties.MessageExpiry) {
			a.LessOrEqual(*pub.Properties.MessageExpiry, uint32(8))
			a.GreaterOrEqual(*pub.Properties.MessageExpiry, uint32(7))
		}
	default:
		t.Fatal("missing output")
	}
	a.Equal(originalExpiry, msg.MessageExpiry)
}
...
func TestClient_connectWithTimeOut_BasicAuth(t *testing.T) {
	var tt = []struct {
		...
		{
			name: "missing_basic_auth_hook_rejects_by_default",
			connect: &packets.Connect{
				Version:    packets.Version311,
				CleanStart: false,
				KeepAlive:  0,
			...
			register: func(connect *packets.Connect, client *client) (sessionResume bool, err error) {
				panic("should not call register without BasicAuth hook")
			},
			ok: false,
			assertConnack: func(a *assert.Assertions, ack *packets.Connack) {
		...
		{
			name: "explicit_anonymous_allows_missing_basic_auth_hook",
			connect: &packets.Connect{
				Version:    packets.Version311,
				CleanStart: false,
				KeepAlive:  0,
	...
	for _, v := range tt {
		t.Run(v.name, func(t *testing.T) {
			...
			srv := defaultServer()
			srv.config.MQTT.AllowZeroLenClientID = v.allowZeroLenClientID
			srv.config.MQTT.AllowAnonymous = v.allowAnonymous
			c, _ := srv.newClient(noopConn{})
			c.in <- v.connect
...
func TestClientConnectWithTimeOutBasicAuthObservesTimeoutContext(t *testing.T) {
	a := assert.New(t)
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()

	previousTimeout := clientConnectTimeout
	clientConnectTimeout = 20 * time.Millisecond
	t.Cleanup(func() { clientConnectTimeout = previousTimeout })

	srv := defaultServer()
	c, _ := srv.newClient(noopConn{})
	c.in <- &packets.Connect{
		Version:    packets.Version311,
		CleanStart: false,
		KeepAlive:  0,
		ClientID:   []byte("cid"),
	}
	c.register = func(connect *packets.Connect, client *client) (bool, error) {
		t.Fatal("register should not be called after BasicAuth timeout")
		return false, nil
	}
	hookEntered := make(chan struct{})
	c.server.hooks.OnBasicAuth = func(ctx context.Context, client Client, req *ConnectRequest) error {
		close(hookEntered)
		<-ctx.Done()
		return ctx.Err()
	}
	...
	case <-hookEntered:
	default:
		t.Fatal("BasicAuth hook was not called")
	}
	select {
	case p := <-c.out:
		t.Fatalf("unexpected packet after BasicAuth timeout: %T", p)
	default:
	}
}
