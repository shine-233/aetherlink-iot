// 文件用途：定义 broker hook 抽象和插件生命周期回调契约。
// 核心逻辑：覆盖接入、认证、连接、会话、订阅、发布、投递、遗嘱、关闭和停机等扩展点。
// 使用注意：hook 触发顺序、参数可变性和 wrapper 语义会影响所有插件，改动必须保持兼容。
// 重构建议：后续可按生命周期拆分 hook 组，并用插件交互测试覆盖兼容性。
